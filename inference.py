...
LEO_IDX = {0, 1, 4, 5}  # config.py에서 LEO용으로 표시된 인덱스
...
def predict_gesture(model, seq, seq_length=WINDOW_SIZE):
    ...
    x = torch.from_numpy(data).unsqueeze(0).to(DEVICE)  # (1,30,99)
    with torch.no_grad():
        logits = model(x)                                 # (1,C)
        probs = torch.softmax(logits, dim=1)
        conf, idx = probs.max(dim=1)                     # (1,), (1,)
            ...
            if cur_seq:
                ...
                if conf and conf >= 0.8:
                    ...
                    if len(pred_queue) == pred_queue.maxlen:
                        ...
                        if count >= 5:
                            cmd = COMMAND[actions.index(most)]
                            if cmd and cmd != pre_cmd:
                                port = ser_leo if actions.index(most) in LEO_IDX else ser_uno
                                port.write((cmd+'\n').encode())
                            pre_cmd = cmd
